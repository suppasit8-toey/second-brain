...
import ImportTeamDialog from './_components/ImportTeamDialog'
...
export default function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const loadData = async () => {
        ...
        if (t) {
            setTournament(t)
            const ownTeams = await getTeams(t.id)
            const linkedTeams = await getScrimPartnerTeams(t.id)
            // effective teams are both
            setTeams([...ownTeams, ...linkedTeams])
        }
    }
        ...
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div>
                ...
                <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-6 rounded-xl">
                    ...
                    {/* Global Action Button Removed - Moved to Sections */}
                </div>
            </div>

            {/* Teams Grid - Participating */}
            <div>
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-bold text-slate-200 flex items-center gap-2">
                        <Users className="w-5 h-5 text-indigo-400" />
                        Participating Teams ({teams.filter(t => t.role !== 'scrim_partner').length})
                    </h2>
                    <Button
                        size="sm"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {teams.filter(t => t.role !== 'scrim_partner').map(team => (
                    ...
                    {teams.filter(t => t.role !== 'scrim_partner').length === 0 && (
                        <div className="col-span-full py-12 text-center text-slate-500 border border-dashed border-slate-800 rounded-xl bg-slate-900/30">
                            No participating teams added yet.
                        </div>
                    )}
                </div>
            </div>

            {/* Teams Grid - Scrim Partners */}
            <div className="pt-8 border-t border-slate-800">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-bold text-slate-200 flex items-center gap-2">
                        <Users className="w-5 h-5 text-emerald-400" />
                        Scrim Partners ({teams.filter(t => t.role === 'scrim_partner').length})
                    </h2>
                    <ImportTeamDialog targetTournamentId={tournament.id} onImportSuccess={loadData} />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {teams.filter(t => t.role === 'scrim_partner').map(team => (
                        <Card key={team.id} className="bg-slate-900 border-slate-800 text-white group hover:border-emerald-500/50 transition-colors">
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <CardTitle className="text-lg font-bold truncate pr-2">{team.name}</CardTitle>
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-600 hover:text-red-400 -mr-2" onClick={() => handleDeleteTeam(team.id)}>
                                    <Trash2 className="w-4 h-4" />
                            ...
                            <CardContent className="space-y-4">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div className="text-xs text-slate-500 uppercase font-bold tracking-wider">Tag</div>
                                        <div className="font-mono text-emerald-400 font-bold">{team.short_name || 'N/A'}</div>
                                    </div>
                                </div>
                                <ManageRosterButton team={team} tournamentId={id} />
                            </CardContent>
